...
1. **Metrics Alert**: Prometheus ตรวจพบ `transfer_failed_total` rate พุ่งเกิน 5% �## Slide 21 — Flow Comparison Overview
...
## Slide 22 — Flow B: Direct to OpenSearch (ไม่มี OTel)
...
## Slide 23 — Flow C: AWS Native (Kinesis Firehose)
...
## Slide 25 — Flow E: Datadog Full Integration
...
## Slide 26 — Flow F: Dynatrace OneAgent Architecture
...
## Slide 27 — Flow G: Splunk Universal Forwarder Architecture
...
| **Frozen** ❄️ | 90+ วัน | S3 SmartStore | Pull on demand |atrace Query Language)
...
## Slide 18 — Cost Control & Data Lifecycle Strategy (ใหม่ ✨)

### 🎯 แนวคิดหลัก
ค่าใช้จ่ายด้าน Observability (Log, Metrics, Traces) ในองค์กรใหญ่ มักจะบานปลายอย่างรวดเร็วจนบางครั้งสูงกว่าค่าระบบงานจริง (Production Run Cost) เสียอีก SRE จำเป็นต้องเข้าใจกลยุทธ์การบริหารจัดการค่าใช้จ่ายผ่านการทำ Filtering, Sampling, Tiering และการแปลงข้อมูลให้เหมาะสมก่อนนำส่ง

---

### 🎛️ 1. Filter & Sample at Source
คัดกรองตั้งแต่ต้นทาง (ที่ Agent เช่น Fluent Bit หรือ OpenTelemetry Collector) เพื่อจำกัดข้อมูลขยะและลบข้อมูลส่วนบุคคล (PII)

- **Log Level Control:** บล็อก `DEBUG` และ `TRACE` ที่ Agent ใน Production รันเฉพาะ `INFO` ขึ้นไป
- **Deduplication / Healthcheck drop:** บล็อก Log ที่ซ้ำซ้อนและไร้ค่า เช่น Traffic จาก Load Balancer ที่มาตรวจสอบ `/healthz` ทุกๆ 1 วินาที
  - *ตัวอย่าง:* การนำเอา log ประเภท GET /healthz ออกไปสามารถลดปริมาณ log ได้ถึง 30-50% ทันทีในระบบ Microservice

---

### 📉 2. Trace & Log Sampling
การเลือกเก็บสถิติแทนการเก็บ 100% (Sampling)

- **Tail-based Sampling:** OTel Collector ช่วยจัดกลุ่มและตรวจสอบสถานะของ Trace ทั้งเส้นก่อนตัดสินใจเก็บ
  - **Success Requests:** เก็บเพียง 5% เพื่อดูภาพรวมเฉลี่ย
  - **Error Requests (5xx, HTTP Timeout):** เก็บเต็ม 100% เสมอ

---

### 🗄️ 3. Multi-tier Log Storage
แบ่งประเภทที่จัดเก็บข้อมูลตามความเร็วในการเรียกใช้และงบประมาณ

1. **Hot Storage (0-7 วัน):** ค้นหาเร็วแบบ real-time คิดราคาแพงสุดต่อ GB (เช่น Datadog Indexed Logs, Elasticsearch SSD)
2. **Warm Storage (7-30 วัน):** ค้นหารวมช้ากว่านิดหน่อย บนดิสก์ราคาถูก (เช่น OpenSearch Warm Instances)
3. **Cold Storage (30-90 วัน):** ย้ายไปจัดเก็บเป็น Object Storage แบบดิบ เช่น S3 หรือ GCS (คิดค่าใช้จ่ายเพียง ~$0.02/GB/Month)
4. **Archive/Frozen Storage (90 วันขึ้นไป):** บีบอัดแล้วจัดเก็บใน S3 Glacier หรือ Google Cloud Archive เพื่อทำ compliance (PCI-DSS / ธปท.) เก็บไว้นาน 5-10 ปี

---

### 📊 4. Log-based Metrics
ลดค่าใช้จ่ายการดึงข้อมูลโดยการเปลี่ยน "Log เป็นตัวเลข"

- แทนที่จะเก็บบันทึก Log เต็มบรรทัดที่มีข้อความยาวๆ เป็นล้านบรรทัดเพื่อทำ dashboard นับจำนวนการ Fail
- ให้ใช้ OTel Collector ตรวจสอบข้อความ error แล้วบวก 1 ลงใน counter metric (`user.login.failed_total`)
- ส่งขึ้นคลาวด์เฉพาะตัวเลข metric ที่สรุปแล้ว ซึ่งมีราคาถูกกว่าการเก็บและ index ข้อมูลดิบแบบ log ถึง 10-20 เท่า

---

### 📋 สรุป Banking App Log Retention Policy (ตัวอย่างการนำเสนอ)
* **Audit Logs (เก็บ 5-10 ปี):** Hot 30 วัน -> Cold 1 ปี -> Glacier 10 ปี (ต้องเก็บตามกฎหมาย)
* **Application Error Logs (เก็บ 90 วัน):** Hot 14 วัน -> Cold 90 วัน -> ลบทิ้ง (ไม่มีเหตุผลต้องเก็บนานกว่านี้)
* **Application Success Logs (เก็บ 30 วัน):** Hot 3 วัน -> Cold 30 วัน -> ลบทิ้ง
* **Infrastructure Metrics (เก็บ 15 เดือน):** เก็บเฉพาะตัวเลขบน Time-series database สำหรับนำไปวิเคราะห์ Capacity Planning รายปี

---

## Slide 19 — Flow Comparison Overview
...
## Slide 20 — Flow B: Direct to OpenSearch (ไม่มี OTel)
